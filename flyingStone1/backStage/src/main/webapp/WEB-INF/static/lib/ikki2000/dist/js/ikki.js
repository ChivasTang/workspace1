...
(function ($) {
    $.fn.ajaxPost = function (options) {
        var defaults = { // 参数默认值
                ...
                ajaxUrl: 'json/response.json', // Gitee Pages 模拟返回的 json 文件，正常使用请改回空字符串
...
function alertMsgBtn(msg, type, closed) {
    ...
    alertWindow.wrapper.find('.k-window-buttongroup .k-button').click(function () {
        if (closed) {
            closed();
        }
        alertWindow.close();
...
function confirmMsgBtn(title, msg, type, confirmed) {
    ...
    confirmWindow.wrapper.find('.k-window-buttongroup .k-state-selected').click(function () {
        confirmed();
    });
    confirmWindow.wrapper.find('.k-window-buttongroup .k-button').click(function () {
        confirmWindow.close();
    });
}

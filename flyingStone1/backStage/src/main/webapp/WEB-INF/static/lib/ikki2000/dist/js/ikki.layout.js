...
$(function () {
    ...
    $('#botChat').kendoChat({
        ...
        {
            type: 'text',
            text: '亲~ 由于码云使用的是HTTPS协议~ 而图灵机器人使用的是HTTP协议~ 所以想看对话效果的亲们请部署到本地开启Chrome跨域模式调试~ 谢谢~ ❤️'
        },
        {
            id: kendo.guid(),
            name: '🌸小艾',
...
function showPath(hash) {
    ...
    // 变更网页标题
    $('head title').text('Kendo UI Admin by IKKI & Amikoko');
    if ($('#path').children('span').length === 0) {
        $('head title').prepend('首页 - ');
    } else {
        $.each($('#path').children('span'), function (i, doms) {
            $('head title').prepend($(doms).text() + ' - ');
        });
    }
}
